...
class GSDRegressor:
    ...
    def __init__(
        ...
        self.smooth_iter = smoothing_iter
        self.smooth_step = smoothing_step
    ...
    def fit(self, X_train: np.ndarray | spmatrix, y_train: np.ndarray | spmatrix) -> None:
        ...
        for i in range(self.max_iter):
            ...
            # Update weight
            if (i > 2 and
                self.smooth_iter != 0 and 
                self.smooth_step != 0 and 
                self.smooth_window != 0 and
                i % self.smooth_iter == 0):
              smoothing = np.mean(self.loss_history[-self.smooth_window]) * 0.1
            ...
            if self.smooth_iter == 0 or self.smooth_step == 0 or self.smooth_window == 0:
              self.loss_history.append(loss)

            else:
              self.loss_history.append(avg_weight)
            ...
            if self.verbose == 1 and ((i % max(1, self.max_iter // 20)) == 0 or i < 5):
                print(f"Epoch {i+1}/{self.max_iter}. Loss: {loss:.6f}, Avg Weights: {avg_weight:.6f}, Avg Bias: {self.b:.6f}")
