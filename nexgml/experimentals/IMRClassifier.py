...
from nexgml.amo import (mean_squared_error, 
                        mean_absolute_error, 
                        smoothl1_loss, 
                        root_squared_error, 
                        softmax)              # For some math operation
from nexgml.indexing import one_hot_labeling  # For one-hot labeling

# ========== THE MODEL ==========
class IMRClassifier:
    """
    Intense Mini-batch Regression Classifier (MRC) is an advanced linear classifier that uses mini-batch gradient descent optimization with softmax for multi-class classification.
    ...
    def __init__(
        ...
        ## Raises:
            **ValueError**: *If invalid loss, penalty, optimizer, or lr_scheduler type is provided, 
    ...
    def _calculate_loss(self, y_true, y_pred_proba):
        ...
        if self.loss == 'mse':
          loss = mean_squared_error(y_true, y_pred_proba)

        elif self.loss == 'rmse':
          loss = root_squared_error(y_true, y_pred_proba)

        elif self.loss == 'mae':
          loss = mean_absolute_error(y_true, y_pred_proba)

        elif self.loss == 'smoothl1':
          loss = smoothl1_loss(y_true, y_pred_proba, self.delta)
    ...
    def _calculate_grad(self, X: np.ndarray | spmatrix, y: np.ndarray, z: np.ndarray | None=None) -> tuple[np.ndarray, np.ndarray]:
        """
        ...
        # RMSE loss gradient
           ...
           grad_w = (X.T @ (2 * error)) / (X.shape[0] * rmse + self.epsilon)
           
           # Calculate bias gradient if intercept is used
           if self.intercept:
              grad_b = np.mean(2 * error) / (rmse + self.epsilon)
        
        # MAE loss gradient
        elif self.loss == 'mae':
           grad_w = X.T @ np.sign(error) / X.shape[0]
