...
class Support:
    def __init__(self,
                ...
                power_t: None=0.25,
                adaptive_alpha: None=False,
                method: None='mean',
                asym_bias: bool=False,
    ...
    def _calculate_loss(self, X, y) -> float:
        ...
        if self.loss not in ("mse", "huber", "rmse"):
    ...
    def grad(self, X: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        if self.loss == 'mse':
           ...
           if self.intercept:
            ...
            grad_w = X.T @ np.sign(f) / X.shape[0]
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray):
        ...
        for i in range(self.max_iter):
            if self.sche:
              self.learning_rate = self.learning_rate / ((i + 1)**self.power_t + 1e-8)
              self.lr_rate_history.append(self.learning_rate)

            if self.ada_alpha and i > 1:
                self.alpha = self.alpha * (self.loss_history[-1] / self.loss_history[-2])
                self.alpha_history.append(self.alpha)

            if self.asym_bias and i % 5 == 0 and i > 10:
               if np.mean(self.loss_history[-5:] ) > np.mean(self.loss_history[-10:-5]):
                   self.weights += self.alpha * np.sign(self.weights)
                   self.b += self.alpha * np.sign(self.b)
            ...
            if self.verbose == 2 and self.verbosity == 'light' and (1 + i) % 10 == 0:
                print(f"|={i + 1}. Grad_w: {np.mean(self.weights):.4f}, Grad_b: {self.b:.4f}, Loss: {loss:.4f}=|")
            
            elif self.verbose == 2 and self.verbosity == 'heavy':
                print(f"|={i + 1}. Grad_w: {np.mean(self.weights):.4f}, Grad_b: {self.b:.4f}, Loss: {loss:.4f}, Alpha: {self.alpha:.4f}, Lr_rate: {self.learning_rate:.4}=|")

            if self.early_stop and i > 1:
                if abs(self.loss_history[-1] - self.loss_history[-2]) < self.tol:
                  break
        if self.method == 'mean':
           return {'weights': self.weights, 'b': self.b, 'loss': np.mean(self.loss_history), 'lr_history': np.mean(self.lr_rate_history) if self.sche else None, 'alpha_history': np.mean(self.alpha_history) if self.ada_alpha else None}
        
        elif self.method == 'last_data':
           return {'weights': self.weights, 'b': self.b, 'loss': self.loss_history[-1], 'lr_history': self.lr_rate_history[-1] if self.sche else None, 'alpha_history': self.alpha_history[-1] if self.ada_alpha else None}
        
        elif self.method == 'min':
           return {'weights': self.weights, 'b': self.b, 'loss': min(self.loss_history), 'lr_history': min(self.lr_rate_history) if self.sche else None, 'alpha_history': min(self.alpha_history) if self.ada_alpha else None}

        elif self.method == 'max':
           return {'weights': self.weights, 'b': self.b, 'loss': max(self.loss_history), 'lr_history': max(self.lr_rate_history) if self.sche else None, 'alpha_history': max(self.alpha_history) if self.ada_alpha else None}


        else:
           raise ValueError(f'Invalid transfer_method argument {self.method}')