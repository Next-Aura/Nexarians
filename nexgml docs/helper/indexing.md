...
This module is part of the internal `NexGML` helper library. You can import the class as follows:

```python
import numpy as np
from nexgml import Indexing 
```
...
### `Indexing.one_hot_labeling(y, classes)`
...
### `Indexing.integer_labeling(y, classes, to_integer_from='one-hot')`
...
### `Indexing.standard_indexing(n, maxi)`
...
# 1. One-Hot Encoding
y_ohe = Indexing.one_hot_labeling(raw_labels, unique_classes)
print(f"One-Hot Encoded:\n{y_ohe}")
# Output: [[0 1 0], [0 0 1], [0 1 0], [1 0 0], [0 0 1]]

# 2. Integer Encoding (from One-Hot)
y_int_from_ohe = Indexing.integer_labeling(y_ohe)
print(f"Integer Indices (from OHE): {y_int_from_ohe}") 
# Output: [1 2 1 0 2] (index 0='bird', 1='cat', 2='dog')

# 3. Integer Encoding (from Labels)
y_int_from_labels = Indexing.integer_labeling(raw_labels, unique_classes, to_integer_from='labels')
...
max_sqrt = Indexing.standard_indexing(total_features, 'sqrt')
print(f"Max features (sqrt of 1000): {max_sqrt}") 
# Output: 31

# Calculate max features based on 5% rule
max_float = Indexing.standard_indexing(total_features, 0.05)
